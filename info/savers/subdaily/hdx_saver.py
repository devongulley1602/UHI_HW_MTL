"""
Loads relative humidity and temperature from zarr
Calculates saturation vapour pressure from temperatures
Uses RH and SVP to get actual vapour pressure
Calculates humidex using actual vapour pressure and temperature
Saves gridded humidex-3hourly data
"""
from Montreal_UHI_toolbox import save_zarr, standard_rechunk, get_outputs 
import numpy as np
import xarray as xr
tas_C,tas_T = get_outputs('tas')    # temperature [K]
hrss_C,hrss_T = get_outputs('hrss') # relative humidity [0-1]
...
# Calculate the actual vapour pressure based on the current hrss (relative humidity) and the SVP at each corresponding temperature sample
