...
def get_outputs(field,extension,canopy='both'):
    ...
    Parameters:
        field : string
            - Name of the 4D field to load ['tas','tasmax','tasmin','hfss','hfls']
        extension : string
    ...
    if '.nc' in extension:
        ...
        match canopy:
            case 'both':
                return xr.open_mfdataset(path_C)[field],xr.open_mfdataset(path_T)[field]
            case 'C':
                return xr.open_mfdataset(path_C)[field]
            case 'T':
                return xr.open_mfdataset(path_T)[field]
    elif '.zarr' in extension:
        path_C = f'{out_dir_C}StLaurent_1km_SL2.5_ERA5_advHU.zarr'
        path_T = f'{out_dir_T}StLaurent_1km_SL2.5_ERA5_advHU_TEB.zarr'
        match canopy:
            case 'both':
                return xr.open_zarr(path_C,mask_and_scale=False)[field],xr.open_zarr(path_T)[field]
            case 'C':
                return xr.open_zarr(path_C,mask_and_scale=False)[field]
            case 'T':
                return xr.open_zarr(path_T,mask_and_scale=False)[field]
    else:
...
def save_zarr(ds,canopy=None,store=None,mode='a-',append_dim='time'):
    """
    ...
    Parameters:
        ...
        mode  : string
            - 'w', 'w-', 'a', 'a-', 'r+', None 
            - Default 'a-' add to the file if it already exists, without overwriting existing variables        
    """
    ...
    if store==None:
        ...
        if canopy == 'C':
            store = f'{out_dir_C}StLaurent_1km_SL2.5_ERA5_advHU.zarr'
        elif canopy == 'T':
            store = f'{out_dir_T}StLaurent_1km_SL2.5_ERA5_advHU_TEB.zarr'
